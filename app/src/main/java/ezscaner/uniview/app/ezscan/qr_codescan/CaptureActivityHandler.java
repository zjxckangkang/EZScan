...
public final class CaptureActivityHandler extends Handler {
    ...
    public CaptureActivityHandler(QRCodeScanAct activity,
    ...
    public void handleMessage(Message message) {
        switch (message.what) {
            case R.id.restart_preview: // 准备进行下一次扫描
                ...
                restartPreviewAndDecode();
                break;
            case R.id.decode_succeeded_byzbar:
                state = State.SUCCESS;
                activity.handleDecode((SymbolSet) message.obj);
                break;
            case R.id.decode_succeeded:
//                Log.d(TAG, "Got decodeLocalPicByZXing succeeded message");
//                state = State.SUCCESS;
...
//                activity.handleDecode((Result) message.obj, barcode, scaleFactor);
            ...
            case R.id.decode_failed:
                // We're decoding as fast as possible, so when one decodeLocalPicByZXing fails,
                // start another.
//                KLog.e("decode_failed");
                state = State.PREVIEW;
