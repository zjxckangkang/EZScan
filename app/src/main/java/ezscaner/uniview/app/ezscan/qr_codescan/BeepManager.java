...
/**
 * Manages beeps and vibrations for {@link QRCodeScanAct}.
 */
public final class BeepManager implements MediaPlayer.OnCompletionListener,
	 ...
	 * @param fileName
	 */
	public BeepManager(Activity activity, String fileName) {
		this.activity = activity;
		this.mediaPlayer = null;
		updatePrefs(fileName);
	}
	 ...
	 * 扫描成功后可以播放提示音并震动，这两种功能都是用户自定义的 在Barcode Scanner中点击菜单键，点设置即可看到这两项的设置
	 */
	public synchronized void updatePrefs(String filename) {
		SharedPreferences prefs = PreferenceManager
				.getDefaultSharedPreferences(activity);
		playBeep = shouldBeep(prefs, activity);
		vibrate = true;
		if (playBeep && mediaPlayer == null) {
			...
			// so we now play on the music stream.
			activity.setVolumeControlStream(AudioManager.STREAM_MUSIC);
			mediaPlayer = buildMediaPlayer(activity, filename);
			AudioManager mAudioManager = (AudioManager) activity
					.getSystemService(Context.AUDIO_SERVICE);
			int maxVolume = mAudioManager
					.getStreamMaxVolume(AudioManager.STREAM_MUSIC);
	...
	private MediaPlayer buildMediaPlayer(Context activity, String fileName) {
		MediaPlayer mediaPlayer = new MediaPlayer();
		mediaPlayer.setAudioStreamType(AudioManager.STREAM_MUSIC);
		mediaPlayer.setOnCompletionListener(this);
		mediaPlayer.setOnErrorListener(this);
		try {
			AssetFileDescriptor file = activity.getResources().getAssets()
					.openFd(fileName);
			mediaPlayer.setDataSource(file.getFileDescriptor(),
					file.getStartOffset(), file.getLength());
			// mediaPlayer.setVolume(BEEP_VOLUME, BEEP_VOLUME);
